...
import { u as useAdminFetch } from './useAdminFetch-BYZzXzfL.mjs';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(paginatedRows), (row) => {
        _push(`<tr><td class="whitespace-nowrap px-4 py-3 text-slate-600">${ssrInterpolate(formatDate(row.createdAt))}</td><td class="px-4 py-3 font-medium text-slate-800">${ssrInterpolate(row.libelle)}</td><td class="whitespace-nowrap px-4 py-3 text-slate-700">${ssrInterpolate(formatMoney(row.tarif))}</td><td class="px-4 py-3"><button type="button" class="${ssrRenderClass([
        ...
        ])}"${ssrIncludeBooleanAttr(unref(togglingId) === row.id) ? " disabled" : ""}>${ssrInterpolate(unref(togglingId) === row.id ? "…" : row.actif ? "Actifs" : "En Arrêt")}</button></td><td class="px-4 py-3 text-right"><div class="inline-flex flex-wrap items-center justify-end gap-2"><button type="button" class="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50"> Modifier </button>`);
        if (row.actif) {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(modalOpen)) {
          _push2(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-[#140C44]/50 p-4 backdrop-blur-md" role="dialog" aria-modal="true"><div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"><h3 class="text-lg font-semibold text-slate-800">${ssrInterpolate(unref(modalMode) === "create" ? "Nouveau métier" : "Modifier métier")}</h3><label class="mt-4 block text-sm font-medium text-slate-700">Libellé</label><input${ssrRenderAttr("value", unref(modalLibelle))} type="text" class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-[#020B51]/40" placeholder="Ex. Plomberie"><label class="mt-4 block text-sm font-medium text-slate-700">Tarif (FCFA)</label><input${ssrRenderAttr("value", unref(modalTarif))} type="text" inputmode="decimal" class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-[#020B51]/40" placeholder="Ex. 5000">`);
          if (unref(modalError)) {
            _push2(`<p class="mt-2 text-sm text-rose-700">${ssrInterpolate(unref(modalError))}</p>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`<div class="mt-6 flex justify-end gap-2"><button type="button" class="rounded-full px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100"> Annuler </button><button type="button" class="rounded-full bg-[#020B51] px-5 py-2 text-sm font-semibold text-white disabled:opacity-50"${ssrIncludeBooleanAttr(unref(modalSaving) || !unref(modalLibelle).trim() || !unref(modalTarif).trim()) ? " disabled" : ""}>${ssrInterpolate(unref(modalSaving) ? "Enregistrement…" : "Enregistrer")}</button></div></div></div>`);
        } else {
          _push2(`<!---->`);
        }
...
//# sourceMappingURL=services-lXhmqCHA.mjs.map
